...
async function probeGateway(input: {
  ...
  return await new Promise((resolve) => {
    ...
    ws.on("message", (raw) => {
      ...
      if (event?.type === "res" && event.ok === true) {
        finish("ok");
      }
    });
      ...
      ws.send(
        JSON.stringify({
          ...
          params: {
            ...
            role: input.role,
            scopes: input.scopes,
            ...(input.authToken ? { auth: { token: input.authToken } } : {}),
          },
        }),
      );
    });
...
export async function testEnvironment(
  ...
  if (!urlValue) {
    checks.push({
      code: "nemoclaw_gateway_url_missing",
      level: "error",
      message: "NemoClaw gateway adapter requires a WebSocket URL.",
  ...
  try {
    ...
    checks.push({
      code: "nemoclaw_gateway_url_invalid",
      level: "error",
  ...
  if (url && url.protocol !== "ws:" && url.protocol !== "wss:") {
    checks.push({
      code: "nemoclaw_gateway_url_protocol_invalid",
      level: "error",
  ...
  if (url) {
    checks.push({
      code: "nemoclaw_gateway_url_valid",
      level: "info",
    ...
    if (url.protocol === "ws:" && !isLoopbackHost(url.hostname)) {
      checks.push({
        code: "nemoclaw_gateway_plaintext_remote_ws",
        level: "warn",
  ...
  if (authToken) {
    checks.push({
      code: "nemoclaw_gateway_auth_present",
      level: "info",
      message: "Gateway credentials are configured.",
    });
  } else {
    checks.push({
      code: "nemoclaw_gateway_auth_missing",
      level: "warn",
      message: "No gateway credentials detected in adapter config.",
      hint: "Set authToken or headers.x-nemoclaw-token for authenticated gateways.",
    });
  }
  ...
  if (url && (url.protocol === "ws:" || url.protocol === "wss:")) {
    try {
      ...
      if (probeResult === "ok") {
        checks.push({
          code: "nemoclaw_gateway_probe_ok",
          level: "info",
          message: "Gateway connect probe succeeded.",
        });
      } else {
        checks.push({
          code: "nemoclaw_gateway_probe_failed",
          level: "warn",
      ...
      checks.push({
        code: "nemoclaw_gateway_probe_error",
        level: "warn",
